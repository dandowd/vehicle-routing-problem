...
func TestSearchForRoutesShouldReturnPathWithLeastTravelCosts2(t *testing.T) {
	...
	if path[0].LoadNumber != 1 {
		t.Errorf("Expected first load to be 1, got %d", path[0].LoadNumber)
	}

	if path[1].LoadNumber != 2 {
		t.Errorf("Expected second load to be 2, got %d", path[1].LoadNumber)
	}
}
